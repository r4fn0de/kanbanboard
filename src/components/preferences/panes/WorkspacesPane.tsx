...
export function WorkspacesPane({
  ...
  const { mutateAsync: setWorkspaceIconPath } =
    useSetWorkspaceIconPathMutation()
  ...
  const handleSaveWorkspace = async (workspaceId: string) => {
    try {
      ...
      if (croppedImageBlob) {
        ...
        console.log(
          'Calling save_cropped_workspace_icon with workspaceId:',
          workspaceId
        )
        ...
        try {
          ...
          throw new Error(
            `Failed to save cropped icon: ${saveError instanceof Error ? saveError.message : String(saveError)}`
          )
        }
  ...
  const handleSelectIcon = async (workspaceId: string) => {
    try {
      ...
      try {
        ...
        toast.error(
          'File not found or cannot be accessed. Please make sure the file exists and you have permission to read it.'
        )
        return
      }
    ...
    (croppedBlob: Blob) => {
      ...
      if (currentEditingWorkspaceId && !editingWorkspace) {
        const workspace = workspaces.find(
          w => w.id === currentEditingWorkspaceId
        )
        if (workspace) {
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Recommended size: 64x64px for best quality
          </p>
        </div>
                    ...
                    {isEditing ? (
                      ...
                      >
                        ...
                        {croppedImagePreview &&
                          (() => {
                            console.log(
                              'Rendering preview section with URL:',
                              croppedImagePreview
                            )
                            return true
                          })() && (
                          ...
                          {workspace.iconPath || croppedImagePreview ? (
                            ...
                            >
                              <div className="rounded-md bg-muted/50 p-3 text-sm text-muted-foreground">
                                &#10024; Color selection is disabled when using{' '}
                                {croppedImagePreview
                                  ? 'a new icon preview'
                                  : 'a custom icon'}
                                .{' '}
                                {croppedImagePreview
                                  ? 'Remove the preview or save changes'
                                  : 'Remove the icon'}{' '}
                                to use colors instead.
                              </div>
                            </motion.div>
                          ) : (
                            <motion.div
                                    ...
                                    {...(shouldReduceMotion
                                      ...
                                      : {
                                          ...
                                          transition: {
                                            ...ANIMATION.SPRING.SMOOTH,
                                            delay:
                                              index * ANIMATION.STAGGER.FAST,
                                          },
                                          whileHover:
                                            ANIMATION.COLOR_BUTTON.whileHover,
                                          whileTap:
                                            ANIMATION.COLOR_BUTTON.whileTap,
                                        })}
                                  />
                                ))}
                              </div>
                        ...
                        >
                          <motion.div
                            {...(shouldReduceMotion ? {} : ANIMATION.BUTTON)}
                          >
                            <Button
                              type="button"
                              size="sm"
                ...
                <>
                  {boards.filter(
                    ...
                    <>
                      ...
                      <strong>{deleteState.workspace?.name}</strong>? This
